...
public class FactCollector {
	...
	protected void addSolrBasicFacts(ResponseBuilder rb, Collection facts) {
		facts.add(rb);
	...
	protected void addFacets(NamedList facetCountsNL, Collection facts) {
		if (facetCountsNL != null) {
			facts.add(facetCountsNL);
		}
	}

	protected void addGrouping(GroupingSpecification groupingSpec, Collection facts) {
		if (groupingSpec != null) {
			facts.add(groupingSpec);
		}
	}

	protected void addSort(SortSpec sortSpec, Collection facts) {
		if (sortSpec != null) {
			facts.add(sortSpec);
		}
	}

	protected void addFilters(List<Query> filters, Collection facts) {
		if (filters != null) {
			facts.add(filters);
	...
	protected void addResults(DocListAndSet results, Collection facts) {
		if (results != null) {
			facts.add(results);
		}
	}

	protected void addQuery(Query theQuery, Collection facts) {
		if (theQuery != null) {
			facts.add(theQuery);
	...
	public void addFacts(AddUpdateCommand cmd, Collection facts) {
		facts.add(cmd);
	...
	public void addFacts(SolrDocument doc, int docId, Collection facts, IndexSchema schema) {
		facts.add(doc);
